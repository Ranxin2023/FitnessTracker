...
const insertNewDB = "insert into ActivitiesTables (id, activity, date, amount) values (?,?,?,?)"
const getOneDB = "select * from ActivitiesTables where activity = ? and date = ?";
const allDB = "select * from ActivitiesTables where activity = ?";
const deletePrevPlannedDB = "DELETE FROM ActivitiesTables WHERE amount < 0 and date BETWEEN ? and ?";
const getMostRecentPrevPlannedDB = "SELECT rowIdNum, activity, MAX(date), amount FROM ActivitiesTables WHERE amount <= 0 and date BETWEEN ? and ?";
const getMostRecentDB = "SELECT MAX(rowIdNum), activity, date, amount FROM ActivitiesTables";
const getPastWeekByActivityDB = "SELECT * FROM ActivitiesTables WHERE id = ? AND activity = ? AND date BETWEEN ? AND ? ORDER BY date ASC";
...
async function post_activity(activity,id) {
  try {
    await db.run(insertNewDB, act.ActivityToList(activity,id));
    let allRecord = await db.all("SELECT * from ActivitiesTables");
    console.log("RETRIVEREMINDER:ALL RECORDS ARE:\n ",allRecord);
  } catch (error) {
 ...
 */
async function get_similar_activities_in_range(usrId,activityType, min, max) {
  try {
    let results = await db.all(getPastWeekByActivityDB, [usrId,activityType, min, max]);
    return results;
  }
  catch (error) {
    console.log(error);
...
async function get_all() {
  try {
    let results = await db.all("select * from ActivitiesTable", []);
    return results;
  } 
  catch (error) {
    console.log(error);
