...
// our database operations
const dbp = require('./user/databaseOpsProfile');
...
app.get('/auth/accepted',
  ...
  function(req, res) {
    ...
    console.log("id: ", usrProfile.id);
    console.log("profile firstName: ", usrProfile.name.givenName);

    // insert infomation of user
    dbp.insertProfile(usrProfile.id, usrProfile.name.givenName);

    // let response = dbp.getName(usrProfile.id);
...
app.get('/reminder', isAuthenticated,
  async function(request, response, next) {
    ...
    if (result != null) {
      ...
      response.send(act.Activity(result,usrProfile.id));
    } else {
...
// Get query to the url /name and respond with the name of the user
// currently logged in
//
// step 8
//
app.get('/name', isAuthenticated,
  async function(req, res) {
    console.log("server received a get /name request at", req.url)
    let name = usrProfile.name.givenName;
    await dbp.getName(name).then(result => {
...
app.post('/store', isAuthenticated,
  async function(request, response, next) {
    console.log("Server recieved a post request at", request.url);
    let id = usrProfile.id;
    let activity = act.Activity(request.body,usrProfile.id)
    
    await dbo.post_activity(activity,id)
...
app.get('/week', isAuthenticated,
  async function(request, response, next) {
    ...
    let max = date
    let result = await dbo.get_similar_activities_in_range(usrProfile.id,activity, min, max)
...
//not allow user get /reminder when not authorized
app.get("/reminder", isAuthenticated,
  (request, response) => {
    response.sendFile(__dirname + "./public/index.html");
  });
...
function gotProfile(accessToken, refreshToken, profile, done) {
  ...
  console.log("usrProfile id: " + userid);
  done(null, userid);
}
